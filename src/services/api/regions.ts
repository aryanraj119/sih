...
export async function fetchRegions(): Promise<ApiResponse<DiscomData[]>> {
  return fetchFromBackend<DiscomData[]>('/api/regions', () => []);
}

// Fetch 9 Delhi analytical regions for spatial map
export async function fetchSpatialRegions(): Promise<ApiResponse<RegionalMapData[]>> {
  const fallback = getFallbackSpatialRegions();
  const result = await fetchFromBackend<{ regions: RegionalMapData[] }>('/api/regions', () => ({ regions: fallback }));
...
export async function fetchRegionsSummary(): Promise<ApiResponse<RegionalSummaryData>> {
  const fallback = getFallbackSummaryData();
  const result = await fetchFromBackend<RegionalSummaryData>('/api/regions/summary', () => fallback);
  if (result.data) {
    return result;
  }
  return { data: fallback, error: result.error, isDemoMode: true };
...
// Fallback synthetic 9 analytical Delhi regions
function getFallbackSpatialRegions(): RegionalMapData[] {
  return [
    ...
    { region_id: 'south_east', region_name: 'South-East Delhi', discom: 'BRPL', current_demand_mw: 880, forecast_demand_mw: 920, forecast_peak_mw: 990, growth_percent: 7.5, solar_generation_mw: 80, net_load_mw: 800, risk_score: 62.0, risk_level: 'HIGH', explanation: 'High commercial peak demand density requiring grid transformer planning.', utilisation_pct: 82.0, peak_time: '15:30' },
...
function getFallbackSummaryData(): RegionalSummaryData {
  return {
    total_current_demand_mw: 9340,
    total_forecast_demand_mw: 9800,
    total_peak_mw: 10600,
    total_solar_mw: 970,
    total_net_load_mw: 8370,
